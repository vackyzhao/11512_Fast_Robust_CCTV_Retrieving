...
#define SAMPLE_IVE_KCF_FILTER_THR 0 /* 把 SCORE 小于 FilterThresh 的删除 */
#define SAMPLE_IVE_KCF_SPATIAL_SCALE (0.0625 * SAMPLE_IVE_QUANT_BASE)
#define SAMPLE_IVE_KCF_NMS_THR (0.5 * SAMPLE_IVE_QUANT_BASE)
#define SAMPLE_IVE_KCF_NUM_BEFORE_NMS 6000
#define SAMPLE_IVE_KCF_SCORE_THR 0.85f // changed from 0.8f -> 0.85f
...
#define SAMPLE_IVE_KCF_CAR_IDX 70
#define SAMPLE_IVE_KCF_PERSON_IDX 15
#define SAMPLE_IVE_KCF_IOU_NMS_THR 0.001f
...
#define SAMPLE_IVE_KCF_RPN_MAX_ROI_NUM 300 /* MaxRoi：最大候选区域数量 */
#define SAMPLE_IVE_KCF_VPSS_MAX_CHN 2
#define SAMPLE_IVE_KCF_MAX_PIC_SIZE 3
#define SAMPLE_IVE_KCF_COLOR 0xFF0000
...
/*
ps32Proposals 一组6个值，分别是
    SAMPLE_IVE_X_MIN_OFFSET     0
    SAMPLE_IVE_Y_MIN_OFFSET     1
    SAMPLE_IVE_X_MAX_OFFSET     2
    SAMPLE_IVE_Y_MAX_OFFSET     3
    SAMPLE_IVE_SCORE_OFFSET     4
    SAMPLE_IVE_SUPPRESS_FLAG_OFFSET   5
FLAG 为 0 表示有效，为 1 表示无效
把 SCORE 小于 FilterThresh 的删除
*/
...
/*
bNoOverlap 如果为 0 表示有重叠（格子），为 1 表示没有重叠
如果 (重叠面积 * 4096) > (总面积 * NmsThresh) 就把 Score 高的保留，低的无效化
*/
...
/*
输入??，输出框 ProposalResult 和框的数量 NumRois
调用了 IVE_FilterLowScoreBbox 删除低分数
调用了 IVE_NonMaxSuppression 删除高重叠
*/
                                 ...
                                 HI_S32 *ps32DstScores, HI_S32 *ps32DstRoi, HI_S32 *ps32ClassRoiNum)
{
     ...
     * 5: The suprressed flag FLAG 为 0 表示有效，为 1 表示无效
     */
    for (j = 0; j < u32ClsScoreChannels; j++)
    {
        for (i = 0; i < u32RoiCnt; i++)
        ...
        for (i = 0; i < u32RoiCnt; i++)
        {
            ...
            ps32ProposalMemPool[u32ProposalMemPoolIndex] =
                ((ps32ProposalMemPool[u32ProposalMemPoolIndex]) > ((HI_S32)u32OriImWidth - 1) ? ((HI_S32)u32OriImWidth - 1) : (ps32ProposalMemPool[u32ProposalMemPoolIndex])) > 0 ? ((ps32ProposalMemPool[u32ProposalMemPoolIndex]) > ((HI_S32)u32OriImWidth) ? (HI_S32)(u32OriImWidth - 1) : (ps32ProposalMemPool[u32ProposalMemPoolIndex])) : 0;
            ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MIN_OFFSET] =
                ((ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MIN_OFFSET]) >
                         ((HI_S32)u32OriImHeight - 1)
                     ? ((HI_S32)u32OriImHeight - 1)
                     : (ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MIN_OFFSET])) > 0
                    ? ((ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MIN_OFFSET]) > ((HI_S32)u32OriImHeight) ? (HI_S32)(u32OriImHeight - 1) : (ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MIN_OFFSET]))
                    : 0;
            ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_X_MAX_OFFSET] =
                ((ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_X_MAX_OFFSET]) >
                    ...
                    ? ((ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_X_MAX_OFFSET]) > ((HI_S32)u32OriImWidth) ? (HI_S32)(u32OriImWidth - 1) : (ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_X_MAX_OFFSET]))
                    : 0;
            ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MAX_OFFSET] =
                ((ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MAX_OFFSET]) >
                         ((HI_S32)u32OriImHeight - 1)
                    ...
                    ? ((ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MAX_OFFSET]) > ((HI_S32)u32OriImHeight) ? (HI_S32)(u32OriImHeight - 1) : (ps32ProposalMemPool[u32ProposalMemPoolIndex + SAMPLE_IVE_Y_MAX_OFFSET]))
                    : 0;
        }
                                        ...
                                        HI_U32 u32BaseHeight, SAMPLE_IVE_KCF_S *pstKcfInfo)
{
    ...
    {
        if (i != SAMPLE_IVE_KCF_CAR_IDX && i != SAMPLE_IVE_KCF_PERSON_IDX)
        {
            continue;
        }
        for (j = 0; j < pstRect->au32RoiNum[i]; j++)
        {
            ...
            k++;
            pstKcfInfo->astRoiInfo[k - 1].u32RoiLifeTime = 10;
...
static HI_VOID *SAMPLE_IVE_Rfcn_Detect(HI_VOID *pArgs)
{
    ...
    BASE_RELEASE:
        s32Ret = HI_MPI_VPSS_ReleaseChnFrame(s32VpssGrp, 2, &stExtFrmInfo); // vpss grp0 chn2
...
static HI_VOID *SAMPLE_IVE_GetFrame(HI_VOID *pArgs)
{
    ...
    while (HI_FALSE == s_bIveKcfStopSignal)
    {
        s32Ret = HI_MPI_VPSS_GetChnFrame(0, 1, &astVideoFrame[0], s32MilliSec); // vpss grp0 chn1
        if (s32Ret != HI_SUCCESS)
        {
            continue;
        }
        ...
        if (CNN_GET_FRM_END == s_stIveKcfInfo.enCnnGetFrmStat)
        {
            ...
            s32Ret = HI_MPI_VPSS_GetChnFrame(0, 2, &astVideoFrame[1], s32MilliSec); // vpss grp0 chn2
            if (s32Ret != HI_SUCCESS)
            {
                SAMPLE_IVE_MUTEX_UNLOCK(s_stIveKcfInfo.GetFrmMutex);
                continue;
    ...
    RELEASE0:
        s32Ret = HI_MPI_VPSS_ReleaseChnFrame(0, 1, &astVideoFrame[0]); // vpss grp0 chn1
        if (s32Ret != HI_SUCCESS)
        {
                                    ...
                                    HI_U32 u32BboxNum, IVE_ROI_INFO_S astNewRoiInfo[], HI_U32 u32MaxRoiNum, HI_U32 *pu32NewRoiNum)
{
    ...
    {
        astRoiInfo[i].u32RoiLifeTime--;
        for (j = 0; j < u32BboxNum; j++)
        {
            HI_BOOL bSuppressed;
            HI_FLOAT f32Iou;

            astBbox[j].stRoiInfo.u32RoiLifeTime--;
        ...
        if (astRoiInfo[i].u32RoiId != 0 && astRoiInfo[i].u32RoiLifeTime > 5)
        {
                                      ...
                                      SAMPLE_RECT_ARRAY_S *pstRect)
{
    ...
    for (i = 0; i < u32BboxObjNum && i < u32BboxNum; i++)
    {
        ...
        */
        ...
        pstRect->astRect[pstRect->u16Num].astPoint[0].s32X = tmp_sx;
        pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y = tmp_sy;
        if ((pstRect->astRect[pstRect->u16Num].astPoint[0].s32X < 0) || (pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y < 0))
        {
            pstRect->astRect[pstRect->u16Num].astPoint[0].s32X = 0;
            pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y = 0;
        }

        HI_S32 MINE_length = SAMPLE_IVE_MAX((astBbox[pstRect->u16Num].stRoiInfo.stRoi.u32Width & (~1)), (astBbox[pstRect->u16Num].stRoiInfo.stRoi.u32Height & (~1)));

        // HI_S32 MINE_margin = SAMPLE_IVE_MIN(SAMPLE_IVE_MIN(pstRect->astRect[i].astPoint[0].s32X, pstRect->astRect[i].astPoint[0].s32Y), (MINE_length / 10 & (-1)));
        // pstRect->astRect[i].astPoint[0].s32X -= MINE_margin;
        // pstRect->astRect[i].astPoint[0].s32Y -= MINE_margin;
        // MINE_length += MINE_margin * 2;
        //printf("[DBG] (x = %d,\t y = %d)\t length = %d\n", pstRect->astRect[pstRect->u16Num].astPoint[0].s32X, pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y, MINE_length);

        pstRect->astRect[pstRect->u16Num].astPoint[SAMPLE_IVE_KCF_IDX_1].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X + MINE_length;
        pstRect->astRect[pstRect->u16Num].astPoint[SAMPLE_IVE_KCF_IDX_1].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y;
        pstRect->astRect[pstRect->u16Num].astPoint[SAMPLE_IVE_KCF_IDX_2].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X + MINE_length;
        pstRect->astRect[pstRect->u16Num].astPoint[SAMPLE_IVE_KCF_IDX_2].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y + MINE_length;
        pstRect->astRect[pstRect->u16Num].astPoint[SAMPLE_IVE_KCF_IDX_3].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X;
        pstRect->astRect[pstRect->u16Num].astPoint[SAMPLE_IVE_KCF_IDX_3].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y + MINE_length;

#define MINE_BOX_X pstRect->astRect[pstRect->u16Num].astPoint[0].s32X
#define MINE_BOX_Xi(a) (pstRect->astRect[(a)].astPoint[0].s32X)
#define MINE_BOX_Y pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y
#define MINE_BOX_Yi(a) (pstRect->astRect[(a)].astPoint[0].s32Y)

        HI_S32 MINE_BoxAreaSum = 0, MINE_BoxAreaInter = 0;
        IVE_Overlap(MINE_BOX_X, MINE_BOX_Y, MINE_BOX_X + MINE_length, MINE_BOX_Y + MINE_length, 0, 0, 1920, 1080, &MINE_BoxAreaSum, &MINE_BoxAreaInter);
        if (MINE_BoxAreaInter < 0.7f * MINE_length * MINE_length)
            continue;

        HI_S32 j = 0, MINE_ContFlag = 0;

        for (j = 0; j < pstRect->u16Num; j++)
        {
            HI_S32 MINE_length_j = pstRect->astRect[j].astPoint[1].s32X - pstRect->astRect[j].astPoint[0].s32X;
            IVE_Overlap(MINE_BOX_X, MINE_BOX_Y, MINE_BOX_X + MINE_length, MINE_BOX_Y + MINE_length,
                        MINE_BOX_Xi(j), MINE_BOX_Yi(j), MINE_BOX_Xi(j) + MINE_length_j, MINE_BOX_Yi(j) + MINE_length_j,
                        &MINE_BoxAreaSum, &MINE_BoxAreaInter);
            if (MINE_BoxAreaInter > 0.1f * MINE_BoxAreaSum)
            {
                if (MINE_length < MINE_length_j)
                {
                    MINE_ContFlag = 1;
                    break;
                }
                ...
                {
                    pstRect->astRect[j].astPoint[0].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X;
                    pstRect->astRect[j].astPoint[0].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y;
                    pstRect->astRect[j].astPoint[SAMPLE_IVE_KCF_IDX_1].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X + MINE_length;
                    pstRect->astRect[j].astPoint[SAMPLE_IVE_KCF_IDX_1].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y;
                    pstRect->astRect[j].astPoint[SAMPLE_IVE_KCF_IDX_2].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X + MINE_length;
                    pstRect->astRect[j].astPoint[SAMPLE_IVE_KCF_IDX_2].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y + MINE_length;
                    pstRect->astRect[j].astPoint[SAMPLE_IVE_KCF_IDX_3].s32X = pstRect->astRect[pstRect->u16Num].astPoint[0].s32X;
                    pstRect->astRect[j].astPoint[SAMPLE_IVE_KCF_IDX_3].s32Y = pstRect->astRect[pstRect->u16Num].astPoint[0].s32Y + MINE_length;
                    MINE_ContFlag = 1;
                    break;
                }
            }
...
static HI_S32 SAMPLE_IVE_DispProcess(VIDEO_FRAME_INFO_S *pstFrameInfo, SAMPLE_RECT_ARRAY_S *pstRect)
{
    ...
    SAMPLE_RECT_ARRAY_S MINE_Rect = *pstRect;
    // if (MINE_Rect.u16Num)
    //     MINE_Rect.u16Num = 1;

    s32Ret = SAMPLE_COMM_VGS_FillRect(pstFrameInfo, &MINE_Rect, SAMPLE_IVE_KCF_COLOR);
    if (s32Ret != HI_SUCCESS)
    {
...
static HI_VOID *SAMPLE_IVE_KcfTracking(HI_VOID *pArgs)
{
    ...
    while (HI_FALSE == s_bIveKcfStopSignal)
    {
        ...
        // s32Ret = MINE_ClassifyProcess(&pstQueueNode->stFrameInfo, &stRect);
        // if (s32Ret != HI_SUCCESS)
        // {
        //     SAMPLE_PRT("Error(%#x), MINE_ClassifyProcess failed!\n", s32Ret);
        // }

    FAIL:
        (HI_VOID) HI_MPI_VPSS_ReleaseChnFrame(0, 1, &pstQueueNode->stFrameInfo); // vpss grp0 chn1
...
static HI_VOID SAMPLE_IVE_KcfDeInit(SAMPLE_IVE_KCF_S *pstIveKcfInfo)
{
    ...
    {
        (HI_VOID) HI_MPI_VPSS_ReleaseChnFrame(0, 2, &pstIveKcfInfo->astFrameInfo[1]); // vpss grp0 chn2
...
static HI_VOID *MINE_UART_Streaming(HI_VOID *pArgs)
{
    char *MINE_String = NULL;
    unsigned char uartReadBuff[2048] = {0}; /* 读取buff大小为2048 */
    unsigned int RecvLen = strlen(MINE_String);
    unsigned int ret = 0;
    int readLen = 0;
    ...
    while (1) {
        if (s_stIveKcfInfo.bNewDetect)
        {
            getStringName(MINE_String);
            ret = UartSend(uartFd, (unsigned char*)MINE_String, strlen(MINE_String));
        }
...
static HI_VOID *MINE_RTSP_Streaming(HI_VOID *pArgs)
{
    HI_S32 i;
    HI_S32 s32Ret;
    SIZE_S stSize[2];
    ...
    ******************************************/
    ...
    /******************************************
     stream save process
    ******************************************/
    s32Ret = SAMPLE_RTSP_VENC_StartGetStream(VencChn, s32ChnNum);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 MineVbConfig(VB_CONFIG_S *pstVbConf)
{
    (HI_VOID) memset_s(pstVbConf, sizeof(VB_CONFIG_S), 0, sizeof(VB_CONFIG_S));
    ...
    enSnsType = s_stViConfig.astViInfo[0].stSnsInfo.enSnsType;
    ...
    u32BlkSize = COMMON_GetPicBufferSize(stSnsSize.u32Width, stSnsSize.u32Height, PIXEL_FORMAT_YVU_SEMIPLANAR_422, DATA_BITWIDTH_8, COMPRESS_MODE_SEG, DEFAULT_ALIGN);
    pstVbConf->astCommPool[0].u64BlkSize = u32BlkSize;
    pstVbConf->astCommPool[0].u32BlkCnt = 10;

    u32BlkSize = COMMON_GetPicBufferSize(720, 576, PIXEL_FORMAT_YVU_SEMIPLANAR_422, DATA_BITWIDTH_8, COMPRESS_MODE_SEG, DEFAULT_ALIGN);
    ...
    //(HI_VOID)memset_s(pstVbConf, sizeof(VB_CONFIG_S), 0, sizeof(VB_CONFIG_S));
    pstVbConf->u32MaxPoolCnt = SAMPLE_IVE_KCF_MAX_POOL_CNT;

    u32BlkSize = COMMON_GetPicBufferSize(astSize[0].u32Width, astSize[0].u32Height, SAMPLE_PIXEL_FORMAT,
                                         DATA_BITWIDTH_8, COMPRESS_MODE_NONE, DEFAULT_ALIGN);
    pstVbConf->astCommPool[0].u64BlkSize = SAMPLE_IVE_MAX(u32BlkSize, pstVbConf->astCommPool[0].u64BlkSize);
    pstVbConf->astCommPool[0].u32BlkCnt = SAMPLE_IVE_KCF_BLK_CNT_10;

    u32BlkSize = VI_GetRawBufferSize(astSize[0].u32Width, astSize[0].u32Height, PIXEL_FORMAT_RGB_BAYER_16BPP,
                                     COMPRESS_MODE_NONE, DEFAULT_ALIGN);
    pstVbConf->astCommPool[SAMPLE_IVE_KCF_IDX_1].u64BlkSize = SAMPLE_IVE_MAX(u32BlkSize, pstVbConf->astCommPool[0].u64BlkSize);
    pstVbConf->astCommPool[SAMPLE_IVE_KCF_IDX_1].u32BlkCnt = SAMPLE_IVE_KCF_BLK_CNT_10; // ori:4
    ...
    u32BlkSize = COMMON_GetPicBufferSize(astSize[SAMPLE_IVE_KCF_IDX_1].u32Width,
                                         ...
                                         DEFAULT_ALIGN);
    pstVbConf->astCommPool[SAMPLE_IVE_KCF_IDX_2].u64BlkSize = SAMPLE_IVE_MAX(u32BlkSize, pstVbConf->astCommPool[0].u64BlkSize);
    pstVbConf->astCommPool[SAMPLE_IVE_KCF_IDX_2].u32BlkCnt = SAMPLE_IVE_KCF_BLK_CNT_10;
    ...
    u32BlkSize = COMMON_GetPicBufferSize(astSize[SAMPLE_IVE_KCF_IDX_2].u32Width,
                                         ...
                                         DEFAULT_ALIGN);
    pstVbConf->astCommPool[SAMPLE_IVE_KCF_IDX_3].u64BlkSize = SAMPLE_IVE_MAX(u32BlkSize, pstVbConf->astCommPool[0].u64BlkSize);
    pstVbConf->astCommPool[SAMPLE_IVE_KCF_IDX_3].u32BlkCnt = SAMPLE_IVE_KCF_BLK_CNT_10;
...
HI_S32 MineVpssConfig(VPSS_GRP_ATTR_S *pstVpssGrpAttr)
{
    HI_S32 i;

    (HI_VOID) memset_s(pstVpssGrpAttr, sizeof(VPSS_GRP_ATTR_S), 0, sizeof(VPSS_GRP_ATTR_S));
    ...
    pstVpssGrpAttr->u32MaxW = astSize[0].u32Width;
    pstVpssGrpAttr->u32MaxH = astSize[0].u32Height;
    pstVpssGrpAttr->bNrEn = HI_FALSE;
    pstVpssGrpAttr->stNrAttr.enCompressMode = COMPRESS_MODE_NONE;
    pstVpssGrpAttr->stNrAttr.enNrMotionMode = NR_MOTION_MODE_NORMAL;

    for (i = 0; i < SAMPLE_IVE_KCF_VPSS_MAX_CHN + 1; i++)
    {
...
{
    HI_CHAR *pcModelName = "./data/rfcn.wk";
    HI_CHAR *pcCnnModelName = "./data/resnet.wk";
    ...
    if (HI_TRUE)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            SAMPLE_PRT("ViConfig failed!\n");
            return;
        }

        /* config vb */
        s32Ret = MineVbConfig(&stVbConf);
        if (s32Ret != HI_SUCCESS)
        {
            SAMPLE_PRT("VbConfig failed!\n");
            return;
        }
        ...
        /* config vpss */
        s32Ret = MineVpssConfig(&stVpssGrpAttr);
        if (s32Ret != HI_SUCCESS)
        {
            SAMPLE_PRT("VbConfig failed!\n");
            return;
        ...
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_0, "CONFIG MIPI FAIL.s32Ret:0x%x\n", s32Ret);

        /*
         * 配置VO参数
         * Config VO parameter
         */
        StVoParamCfg(&s_stVoConfig);

        // /* config vo */
        // SAMPLE_COMM_VO_GetDefConfig(&s_stVoConfig);
        // s_stVoConfig.enDstDynamicRange = enDynamicRange; //(DYNAMIC_RANGE_SDR8)
         ...
         */
        s32Ret = SampleCommVoStartMipi(&s_stVoConfig);
        SAMPLE_CHECK_EXPR_GOTO(s32Ret != HI_SUCCESS, END_3, "start vo FAIL. s32Ret: 0x%x\n", s32Ret);
    ...
    prctl(PR_SET_NAME, "RtspStreaming", 0, 0, 0);
    pthread_create(&s_RtspStreamingThread, 0, MINE_RTSP_Streaming, NULL);
    pthread_create(&s_RtspStreamingThread, 0, MINE_UART_Streaming, NULL);
    pthread_detach(&s_RtspStreamingThread);
